import {
  dynamoDbRequest,
  isDynamoCredentialsConfigured,
} from './dynamoDb.js';

function getInventoryTableName() {
  return process.env.DYNAMODB_INVENTORY_TABLE || '';
}

export function isDynamoInventoryConfigured() {
  return !!getInventoryTableName();
}

function normalizeQuantity(quantity) {
  const value = Number(quantity);
  return Math.max(0, Number.isFinite(value) ? Math.trunc(value) : 0);
}

function normalizeInventoryRow(row = {}) {
  const sku = String(row.sku || row.sku_key || '').trim();
  const quantity = normalizeQuantity(row.quantity);
  const name = String(row.name || row.product_name || '').trim();
  const hidden = row.hidden === true || row.hidden === 'true';
  return {
    sku,
    sku_key: sku,
    name,
    hidden,
    quantity,
    updated_at: row.updated_at || null,
    attention_muted: row.attention_muted === true,
    attention_muted_at: row.attention_muted_at || null,
    attention_muted_for_updated_at:
      row.attention_muted_for_updated_at ?? null,
  };
}

function fromDynamoItem(item = {}) {
  return normalizeInventoryRow({
    sku: item.sku?.S,
    name: item.name?.S,
    hidden: item.hidden?.BOOL,
    quantity: item.quantity?.N,
    updated_at: item.updated_at?.S || null,
    attention_muted: item.attention_muted?.BOOL,
    attention_muted_at: item.attention_muted_at?.S || null,
    attention_muted_for_updated_at:
      item.attention_muted_for_updated_at?.S ?? null,
  });
}

function inventoryUpdatedAtCondition(expectedUpdatedAt) {
  return expectedUpdatedAt
    ? '#updated_at = :expected_updated_at'
    : 'attribute_not_exists(#updated_at)';
}

function inventoryUpdateTransaction({
  tableName,
  sku,
  quantity,
  name,
  hidden,
  expectedUpdatedAt,
  updatedAt,
}) {
  return {
    Update: {
      TableName: tableName,
      Key: { sku: { S: sku } },
      UpdateExpression:
        'SET #quantity = :quantity, #name = :name, #hidden = :hidden, #updated_at = :updated_at',
      ConditionExpression: `attribute_exists(#sku) AND ${inventoryUpdatedAtCondition(
        expectedUpdatedAt
      )}`,
      ExpressionAttributeNames: {
        '#sku': 'sku',
        '#quantity': 'quantity',
        '#name': 'name',
        '#hidden': 'hidden',
        '#updated_at': 'updated_at',
      },
      ExpressionAttributeValues: {
        ':quantity': { N: String(quantity) },
        ':name': { S: name },
        ':hidden': { BOOL: hidden },
        ':updated_at': { S: updatedAt },
        ...(expectedUpdatedAt
          ? { ':expected_updated_at': { S: expectedUpdatedAt } }
          : {}),
      },
    },
  };
}

function inventoryCreateTransaction({
  tableName,
  sku,
  quantity,
  name,
  hidden,
  updatedAt,
}) {
  return {
    Put: {
      TableName: tableName,
      Item: {
        sku: { S: sku },
        quantity: { N: String(quantity) },
        name: { S: name },
        hidden: { BOOL: hidden },
        updated_at: { S: updatedAt },
      },
      ConditionExpression: 'attribute_not_exists(#sku)',
      ExpressionAttributeNames: { '#sku': 'sku' },
    },
  };
}

function inventoryHideTransaction({
  tableName,
  sku,
  expectedUpdatedAt,
  updatedAt,
}) {
  return {
    Update: {
      TableName: tableName,
      Key: { sku: { S: sku } },
      UpdateExpression: 'SET #hidden = :hidden, #updated_at = :updated_at',
      ConditionExpression: `attribute_exists(#sku) AND ${inventoryUpdatedAtCondition(
        expectedUpdatedAt
      )}`,
      ExpressionAttributeNames: {
        '#sku': 'sku',
        '#hidden': 'hidden',
        '#updated_at': 'updated_at',
      },
      ExpressionAttributeValues: {
        ':hidden': { BOOL: true },
        ':updated_at': { S: updatedAt },
        ...(expectedUpdatedAt
          ? { ':expected_updated_at': { S: expectedUpdatedAt } }
          : {}),
      },
    },
  };
}

export function buildCatalogInventoryTransactItems(
  product,
  previous,
  inventoryRows = [],
  options = {}
) {
  const tableName =
    String(options.tableName || '').trim() || getInventoryTableName();
  if (!tableName) {
    throw new Error('DYNAMODB_INVENTORY_TABLE is not configured on the server');
  }

  const updatedAt = String(options.updatedAt || new Date().toISOString());
  const rowsBySku = new Map(
    inventoryRows
      .map(normalizeInventoryRow)
      .filter((row) => row.sku)
      .map((row) => [row.sku, row])
  );
  const currentSkus = new Set();
  const transactItems = [];

  for (const entry of product?.skuEntries || []) {
    const sku = String(entry?.sku || '').trim();
    if (!sku || currentSkus.has(sku)) continue;
    currentSkus.add(sku);

    const quantity = normalizeQuantity(entry.quantity);
    const name = String(entry.label || product?.name || sku).trim() || sku;
    const hidden = entry.hidden === true || entry.active === false;
    const row = rowsBySku.get(sku);

    if (!row) {
      transactItems.push(
        inventoryCreateTransaction({
          tableName,
          sku,
          quantity,
          name,
          hidden,
          updatedAt,
        })
      );
      continue;
    }

    const unchanged =
      row.quantity === quantity &&
      row.hidden === hidden &&
      row.name === name;
    if (unchanged) continue;

    const expectedUpdatedAt = String(entry.inventoryUpdatedAt || '');
    if (expectedUpdatedAt !== String(row.updated_at || '')) {
      throw new Error(
        `Inventory changed for ${sku}. Refresh before saving this product.`
      );
    }

    transactItems.push(
      inventoryUpdateTransaction({
        tableName,
        sku,
        quantity,
        name,
        hidden,
        expectedUpdatedAt,
        updatedAt,
      })
    );
  }

  for (const previousEntry of previous?.skuEntries || []) {
    const sku = String(previousEntry?.sku || '').trim();
    if (!sku || currentSkus.has(sku)) continue;
    const row = rowsBySku.get(sku);
    if (!row || row.hidden) continue;

    transactItems.push(
      inventoryHideTransaction({
        tableName,
        sku,
        expectedUpdatedAt: String(row.updated_at || ''),
        updatedAt,
      })
    );
  }

  return transactItems;
}

async function listInventoryFromDynamo() {
  const tableName = getInventoryTableName();
  const rows = [];
  let exclusiveStartKey;

  do {
    const response = await dynamoDbRequest('Scan', {
      TableName: tableName,
      ProjectionExpression:
        '#sku, #name, #hidden, #quantity, #updated_at, #attention_muted, #attention_muted_at, #attention_muted_for_updated_at',
      ExpressionAttributeNames: {
        '#sku': 'sku',
        '#name': 'name',
        '#hidden': 'hidden',
        '#quantity': 'quantity',
        '#updated_at': 'updated_at',
        '#attention_muted': 'attention_muted',
        '#attention_muted_at': 'attention_muted_at',
        '#attention_muted_for_updated_at':
          'attention_muted_for_updated_at',
      },
      ...(exclusiveStartKey ? { ExclusiveStartKey: exclusiveStartKey } : {}),
    });

    for (const item of response.Items || []) {
      rows.push(fromDynamoItem(item));
    }

    exclusiveStartKey = response.LastEvaluatedKey;
  } while (exclusiveStartKey);

  return rows.sort((a, b) => a.sku.localeCompare(b.sku));
}

async function getInventoryForSkusFromDynamo(skus) {
  const tableName = getInventoryTableName();
  const keys = skus.map((sku) => ({ sku: { S: String(sku) } }));
  const rows = [];

  for (let i = 0; i < keys.length; i += 100) {
    let pendingKeys = keys.slice(i, i + 100);
    let attempts = 0;

    while (pendingKeys.length) {
      const response = await dynamoDbRequest('BatchGetItem', {
        RequestItems: {
          [tableName]: {
            Keys: pendingKeys,
            ProjectionExpression:
              '#sku, #name, #hidden, #quantity, #updated_at, #attention_muted, #attention_muted_at, #attention_muted_for_updated_at',
            ExpressionAttributeNames: {
              '#sku': 'sku',
              '#name': 'name',
              '#hidden': 'hidden',
              '#quantity': 'quantity',
              '#updated_at': 'updated_at',
              '#attention_muted': 'attention_muted',
              '#attention_muted_at': 'attention_muted_at',
              '#attention_muted_for_updated_at':
                'attention_muted_for_updated_at',
            },
          },
        },
      });

      for (const item of response.Responses?.[tableName] || []) {
        rows.push(fromDynamoItem(item));
      }

      pendingKeys =
        response.UnprocessedKeys?.[tableName]?.Keys || [];
      attempts += 1;
      if (pendingKeys.length && attempts >= 5) {
        throw new Error(
          'Inventory lookup remained throttled after five attempts'
        );
      }
      if (pendingKeys.length) {
        await new Promise((resolve) =>
          setTimeout(resolve, 25 * 2 ** (attempts - 1))
        );
      }
    }
  }

  return rows;
}

async function setInventoryQuantityInDynamo(sku, quantity, options = {}) {
  const tableName = getInventoryTableName();
  const updatedAt = new Date().toISOString();
  const cleanSku = String(sku).trim();
  const cleanQuantity = normalizeQuantity(quantity);
  const name = String(options.name || '').trim();

  const expressionNames = {
    '#quantity': 'quantity',
    '#updated_at': 'updated_at',
  };
  const expressionValues = {
    ':quantity': { N: String(cleanQuantity) },
    ':updated_at': { S: updatedAt },
  };
  const setParts = ['#quantity = :quantity', '#updated_at = :updated_at'];
  const createOnly = options.createOnly === true;
  const hasExpectedUpdatedAt = Object.prototype.hasOwnProperty.call(
    options,
    'expectedUpdatedAt'
  );
  const expectedUpdatedAt = String(options.expectedUpdatedAt || '');
  let conditionExpression = '';

  if (createOnly) {
    expressionNames['#sku'] = 'sku';
    conditionExpression = 'attribute_not_exists(#sku)';
  } else if (hasExpectedUpdatedAt) {
    expressionNames['#sku'] = 'sku';
    expressionValues[':expected_updated_at'] = { S: expectedUpdatedAt };
    conditionExpression = expectedUpdatedAt
      ? 'attribute_exists(#sku) AND #updated_at = :expected_updated_at'
      : 'attribute_exists(#sku) AND (attribute_not_exists(#updated_at) OR #updated_at = :expected_updated_at)';
  }

  if (name) {
    expressionNames['#name'] = 'name';
    expressionValues[':name'] = { S: name };
    setParts.push('#name = :name');
  }

  if (typeof options.hidden === 'boolean') {
    expressionNames['#hidden'] = 'hidden';
    expressionValues[':hidden'] = { BOOL: options.hidden };
    setParts.push('#hidden = :hidden');
  }

  const response = await dynamoDbRequest('UpdateItem', {
    TableName: tableName,
    Key: { sku: { S: cleanSku } },
    UpdateExpression: `SET ${setParts.join(', ')}`,
    ExpressionAttributeNames: expressionNames,
    ExpressionAttributeValues: expressionValues,
    ...(conditionExpression
      ? { ConditionExpression: conditionExpression }
      : {}),
    ReturnValues: 'ALL_NEW',
  });

  return fromDynamoItem(response.Attributes);
}

async function setInventoryHiddenInDynamo(sku, hidden, options = {}) {
  const tableName = getInventoryTableName();
  const updatedAt = new Date().toISOString();
  const hasExpectedUpdatedAt = Object.prototype.hasOwnProperty.call(
    options,
    'expectedUpdatedAt'
  );
  const expectedUpdatedAt = String(options.expectedUpdatedAt || '');
  const response = await dynamoDbRequest('UpdateItem', {
    TableName: tableName,
    Key: { sku: { S: String(sku) } },
    UpdateExpression: 'SET #hidden = :hidden, #updated_at = :updated_at',
    ExpressionAttributeNames: {
      '#sku': 'sku',
      '#hidden': 'hidden',
      '#updated_at': 'updated_at',
    },
    ExpressionAttributeValues: {
      ':hidden': { BOOL: !!hidden },
      ':updated_at': { S: updatedAt },
      ...(hasExpectedUpdatedAt
        ? { ':expected_updated_at': { S: expectedUpdatedAt } }
        : {}),
    },
    ConditionExpression: hasExpectedUpdatedAt
      ? expectedUpdatedAt
        ? 'attribute_exists(#sku) AND #updated_at = :expected_updated_at'
        : 'attribute_exists(#sku) AND (attribute_not_exists(#updated_at) OR #updated_at = :expected_updated_at)'
      : 'attribute_exists(#sku)',
    ReturnValues: 'ALL_NEW',
  });

  return fromDynamoItem(response.Attributes);
}

async function setInventoryAttentionMutedInDynamo(
  sku,
  muted,
  options = {}
) {
  const tableName = getInventoryTableName();
  const cleanSku = String(sku).trim();
  const expectedUpdatedAt = String(options.expectedUpdatedAt || '');
  const expressionAttributeNames = {
    '#sku': 'sku',
    '#updated_at': 'updated_at',
    '#attention_muted': 'attention_muted',
    '#attention_muted_at': 'attention_muted_at',
    '#attention_muted_for_updated_at': 'attention_muted_for_updated_at',
  };
  const expressionAttributeValues = {
    ':expected_updated_at': { S: expectedUpdatedAt },
  };
  const conditionExpression = expectedUpdatedAt
    ? 'attribute_exists(#sku) AND #updated_at = :expected_updated_at'
    : 'attribute_exists(#sku) AND (attribute_not_exists(#updated_at) OR #updated_at = :expected_updated_at)';

  if (muted) {
    expressionAttributeValues[':attention_muted'] = { BOOL: true };
    expressionAttributeValues[':attention_muted_at'] = {
      S: new Date().toISOString(),
    };
    expressionAttributeValues[':attention_muted_for_updated_at'] = {
      S: expectedUpdatedAt,
    };
  }

  const response = await dynamoDbRequest('UpdateItem', {
    TableName: tableName,
    Key: { sku: { S: cleanSku } },
    UpdateExpression: muted
      ? 'SET #attention_muted = :attention_muted, #attention_muted_at = :attention_muted_at, #attention_muted_for_updated_at = :attention_muted_for_updated_at'
      : 'REMOVE #attention_muted, #attention_muted_at, #attention_muted_for_updated_at',
    ConditionExpression: conditionExpression,
    ExpressionAttributeNames: expressionAttributeNames,
    ExpressionAttributeValues: expressionAttributeValues,
    ReturnValues: 'ALL_NEW',
  });

  return fromDynamoItem(response.Attributes);
}

async function applyInventoryDeltaInDynamo(sku, delta) {
  const tableName = getInventoryTableName();
  const cleanSku = String(sku).trim();
  const cleanDelta = Math.trunc(Number(delta));
  const updatedAt = new Date().toISOString();

  if (cleanDelta >= 0) {
    const response = await dynamoDbRequest('UpdateItem', {
      TableName: tableName,
      Key: { sku: { S: cleanSku } },
      UpdateExpression: 'SET #updated_at = :updated_at ADD #quantity :delta',
      ExpressionAttributeNames: {
        '#quantity': 'quantity',
        '#updated_at': 'updated_at',
      },
      ExpressionAttributeValues: {
        ':delta': { N: String(cleanDelta) },
        ':updated_at': { S: updatedAt },
      },
      ReturnValues: 'ALL_NEW',
    });

    return fromDynamoItem(response.Attributes);
  }

  try {
    const response = await dynamoDbRequest('UpdateItem', {
      TableName: tableName,
      Key: { sku: { S: cleanSku } },
      UpdateExpression: 'SET #updated_at = :updated_at ADD #quantity :delta',
      ConditionExpression: 'attribute_exists(#sku) AND #quantity >= :required',
      ExpressionAttributeNames: {
        '#sku': 'sku',
        '#quantity': 'quantity',
        '#updated_at': 'updated_at',
      },
      ExpressionAttributeValues: {
        ':delta': { N: String(cleanDelta) },
        ':required': { N: String(Math.abs(cleanDelta)) },
        ':updated_at': { S: updatedAt },
      },
      ReturnValues: 'ALL_NEW',
    });

    return fromDynamoItem(response.Attributes);
  } catch (err) {
    if (String(err.message).toLowerCase().includes('conditional')) {
      return setInventoryQuantityInDynamo(cleanSku, 0);
    }
    throw err;
  }
}

function shouldUseDynamo() {
  return isDynamoInventoryConfigured();
}

function assertDynamoInventoryReady() {
  if (!shouldUseDynamo()) {
    throw new Error('DYNAMODB_INVENTORY_TABLE is not configured on the server');
  }
  if (!isDynamoCredentialsConfigured()) {
    throw new Error('AWS credentials are not configured for DynamoDB');
  }
}

export async function listInventory() {
  assertDynamoInventoryReady();
  return listInventoryFromDynamo();
}

export async function getInventoryForSkus(skus = []) {
  const cleanSkus = [...new Set(
    (Array.isArray(skus) ? skus : [])
      .map((sku) => String(sku || '').trim())
      .filter(Boolean)
  )];

  if (!cleanSkus.length) return [];

  assertDynamoInventoryReady();
  return getInventoryForSkusFromDynamo(cleanSkus);
}

export async function setInventoryQuantity(sku, quantity, options = {}) {
  const cleanSku = String(sku || '').trim();
  if (!cleanSku) throw new Error('SKU is required');

  assertDynamoInventoryReady();
  return setInventoryQuantityInDynamo(cleanSku, quantity, options);
}

export async function setInventoryHidden(sku, hidden, options = {}) {
  const cleanSku = String(sku || '').trim();
  if (!cleanSku) throw new Error('SKU is required');

  assertDynamoInventoryReady();
  return setInventoryHiddenInDynamo(cleanSku, hidden, options);
}

export async function setInventoryAttentionMuted(
  sku,
  muted,
  options = {}
) {
  const cleanSku = String(sku || '').trim();
  if (!cleanSku) throw new Error('SKU is required');
  if (!Object.prototype.hasOwnProperty.call(options, 'expectedUpdatedAt')) {
    throw new Error('The inventory version is required');
  }

  assertDynamoInventoryReady();
  return setInventoryAttentionMutedInDynamo(
    cleanSku,
    muted === true,
    options
  );
}

export async function applyInventoryDelta(sku, delta) {
  const cleanSku = String(sku || '').trim();
  const cleanDelta = Number(delta);
  if (!cleanSku) throw new Error('SKU is required');
  if (!Number.isFinite(cleanDelta)) throw new Error('Inventory delta is invalid');

  assertDynamoInventoryReady();
  return applyInventoryDeltaInDynamo(cleanSku, cleanDelta);
}

async function deleteInventorySkuFromDynamo(sku, options = {}) {
  const tableName = getInventoryTableName();
  const hasExpectedUpdatedAt = Object.prototype.hasOwnProperty.call(
    options,
    'expectedUpdatedAt'
  );
  const expectedUpdatedAt = String(options.expectedUpdatedAt || '');
  await dynamoDbRequest('DeleteItem', {
    TableName: tableName,
    Key: { sku: { S: String(sku) } },
    ConditionExpression: hasExpectedUpdatedAt
      ? expectedUpdatedAt
        ? 'attribute_exists(#sku) AND #updated_at = :expected_updated_at'
        : 'attribute_exists(#sku) AND (attribute_not_exists(#updated_at) OR #updated_at = :expected_updated_at)'
      : 'attribute_exists(#sku)',
    ExpressionAttributeNames: {
      '#sku': 'sku',
      ...(hasExpectedUpdatedAt ? { '#updated_at': 'updated_at' } : {}),
    },
    ...(hasExpectedUpdatedAt
      ? {
          ExpressionAttributeValues: {
            ':expected_updated_at': { S: expectedUpdatedAt },
          },
        }
      : {}),
  });
  return { sku: String(sku), deleted: true };
}

export async function deleteInventorySku(sku, options = {}) {
  const cleanSku = String(sku || '').trim();
  if (!cleanSku) throw new Error('SKU is required');

  assertDynamoInventoryReady();
  return deleteInventorySkuFromDynamo(cleanSku, options);
}
