...
export async function getEpisodeStudiosByIds(episodeIds = []) {
  if (!isEpisodeStudioStoreConfigured()) {
    return { episodes: [], configured: false };
  }
  const ids = [
    ...new Set(
      (Array.isArray(episodeIds) ? episodeIds : [])
  ...
  for (let index = 0; index < ids.length; index += 100) {
    let keys = ids.slice(index, index + 100).map((episodeId) => ({
      content_key: { S: episodeKey(episodeId) },
    }));
    for (let attempt = 0; keys.length && attempt < 3; attempt += 1) {
      const response = await dynamoDbRequest('BatchGetItem', {
        RequestItems: {
          [getSiteContentTableName()]: {
            Keys: keys,
            ProjectionExpression: '#key, #content_json, #updated_at',
            ExpressionAttributeNames: {
              '#key': 'content_key',
      ...
      for (
        const item of response.Responses?.[getSiteContentTableName()] || []
      ) {
        const episode = parseEpisode(item);
        if (episode?.episode_id) episodes.push(episode);
      }
      keys =
        response.UnprocessedKeys?.[getSiteContentTableName()]?.Keys || [];
    }
  }
...
export async function deleteEpisodeStudio(episodeId, options = {}) {
  if (!isEpisodeStudioStoreConfigured()) {
    throw new Error('Episode Studio storage is not configured.');
  }
  ...
  if (!cleanEpisodeId || !expectedUpdatedAt) {
    throw new Error(
      'Episode Studio: deletion requires the current studio version.'
    );
  }
