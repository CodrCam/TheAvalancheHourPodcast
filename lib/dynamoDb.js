...
export function isDynamoCredentialsConfigured() {
  ...
  if (dynamoAccessKeyId || dynamoSecretAccessKey) {
    return !!(dynamoAccessKeyId && dynamoSecretAccessKey);
  }

  return !!(readEnv('AWS_ACCESS_KEY_ID') && readEnv('AWS_SECRET_ACCESS_KEY'));
}
...
export async function dynamoDbRequest(action, body) {
  ...
  const accessKeyId = usingDynamoNamedCredentials
    ? dynamoAccessKeyId
    : readEnv('AWS_ACCESS_KEY_ID');
  const secretAccessKey = usingDynamoNamedCredentials
    ? dynamoSecretAccessKey
    : readEnv('AWS_SECRET_ACCESS_KEY');
  const sessionToken = usingDynamoNamedCredentials
