...
export const EPISODE_PRODUCTION_PLAN_SCHEMA_VERSION = 2;
export const EPISODE_PRODUCTION_DEADLINE_SCHEMA_VERSION = 2;
export const INTRO_RECORDING_LATEST_DAYS_BEFORE_AIR = 10;
...
const EARLIER_DEFAULT_DEADLINE_BY_TASK_ID = Object.freeze({
  'show-notes-brief': { from: 7, to: 10 },
  'producer-proof-upload': { from: 7, to: 10 },
  'proof-listen-approval': { from: 5, to: 8 },
  'publishing-package': { from: 4, to: 7 },
  'promotion-scheduled': { from: 3, to: 7 },
  'guest-assets-shared': { from: 3, to: 7 },
});
...
export const DEFAULT_EPISODE_PRODUCTION_TASKS = [
  ...
  },
  {
    ...
    label: 'Deliver the recording and edit package',
    description:
    ...
    kind: 'standard',
    linked_deliverable_ids: ['recording-files', 'edit-notes'],
    subtasks: [],
    sort_order: 30,
  },
  {
    ...
    kind: 'intro',
    linked_deliverable_ids: ['intro-audio'],
    subtasks: [],
    sort_order: 40,
  },
  {
    task_id: 'show-notes-brief',
    label: 'Deliver the show-notes and promotion brief',
    description:
  ...
  },
  {
    ...
    label: 'Finish the publishing package',
    description:
  ...
  },
  {
    ...
    label: 'Schedule episode promotion',
    description:
...
export function normalizeEpisodeProductionTasks(tasks, airDate, options = {}) {
  const normalizedAirDate = resolveAirDate(airDate);
  const source = Array.isArray(tasks) ? tasks.slice(0, MAX_PRODUCTION_TASKS) : [];
  const sourceById = new Map();
...
export function isEpisodeProductionTaskComplete(
  ...
  if (task.kind === 'intro') {
    if (task.intro_method === 'recorded') {
      return hasDeliverableAsset(
        episode,
        task.linked_deliverable_ids || ['intro-audio'],
  ...
  if (task.task_id === MICROPHONE_PLAN_TASK_ID) {
    ...
    const plansByHost = new Map(
      (Array.isArray(microphonePlan.mic_kit_plans)
        ? microphonePlan.mic_kit_plans
        : []
      )
        .map((plan) => [cleanId(plan?.host_person_id), plan])
        .filter(([hostPersonId]) => hostPersonId)
    );
    return (
      hostPersonIds.length > 0 &&
      hostPersonIds.every((hostPersonId) => {
        const plan = plansByHost.get(hostPersonId) || {};
        if (plan.choice === 'request_kit') {
          return Boolean(cleanId(plan.request_id));
        }
        if (plan.choice === 'use_own_equipment') {
          return Boolean(cleanText(plan.equipment_note, 800));
        }
...
function validateTaskCompletion(task, episode, taskById, options = {}) {
  ...
  if (task.task_id === 'intro-ready') {
    if (task.intro_method === 'recorded') {
      throw new Error(
        'Episode production: upload the recorded intro before completing this step.'
      );
    }
    if (task.intro_method === 'scheduled_with_producer') {
      throw new Error(
        'Episode production: schedule the producer recording no later than ten days before air.'
      );
    }
    throw new Error(
  ...
  if (task.task_id === MICROPHONE_PLAN_TASK_ID) {
    throw new Error(
      'Episode production: every assigned host must complete the microphone plan before completing this step.'
    );
  }
  if (task.task_id === 'producer-proof-upload') {
    throw new Error(
