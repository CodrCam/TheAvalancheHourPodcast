...
function normalizeKit(value = {}, index = 0) {
  ...
  return {
    ...
    status: MIC_KIT_STATUSES.includes(status)
      ? status
...
export function micKitTrackerSummary(trackerValue) {
  ...
  return {
    ...
    moving: activeKits.filter((kit) =>
      ['in_transit', 'returning'].includes(kit.status)
    ).length,
    with_team: activeKits.filter((kit) =>
      ['reserved', 'with_holder'].includes(kit.status)
    ).length,
    needs_attention: activeKits.filter((kit) =>
