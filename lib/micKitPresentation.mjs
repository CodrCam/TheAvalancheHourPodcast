...
export function sanitizeMicKitTrackerForViewer(trackerValue, viewer = {}) {
  ...
  return {
    ...
    kits: tracker.kits.map((kit) => {
      const canSeeCustody =
        canManage ||
        relatedRequestIds.has(kit.next_request_id) ||
        relatedRequestIds.has(kit.checked_out_request_id);
      return {
        ...kit,
        details_visible: canSeeCustody,
        current_holder_name: canSeeCustody ? kit.current_holder_name : '',
        current_location: canSeeCustody ? kit.current_location : '',
        next_request_id: canSeeCustody ? kit.next_request_id : '',
        ship_by: canSeeCustody ? kit.ship_by : '',
        carrier: canSeeCustody ? kit.carrier : '',
        tracking_number: canSeeCustody ? kit.tracking_number : '',
        tracking_url: canSeeCustody ? kit.tracking_url : '',
        tracking_available:
          canSeeCustody && Boolean(kit.tracking_number || kit.tracking_url),
        checked_out_request_id: canSeeCustody
          ? kit.checked_out_request_id
          : '',
        checked_out_at: canSeeCustody ? kit.checked_out_at : '',
        due_back: canSeeCustody ? kit.due_back : '',
