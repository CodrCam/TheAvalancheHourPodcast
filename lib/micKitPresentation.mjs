...
function requestIsCoordinatedByViewer(request, viewer = {}) {
  ...
  // are authoritative. A person removed from an episode must not retain guest
  // address access through an older stored coordinator snapshot.
  if (
    request.participant_type === 'guest' &&
    hasCurrentEpisodeScope
  ) {
    return Boolean(
      personId &&
        request.episode_id &&
  ...
  if (storedCoordinator) return true;
  return Boolean(
    request.participant_type === 'guest' &&
      request.episode_id &&
      coordinatedEpisodeIds.has(request.episode_id)
