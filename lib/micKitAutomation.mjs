...
function plannedDueBack(request) {
  return (
...
function assignmentAvailability(tracker, kit, request, today) {
  const shipBy = plannedShipBy(kit, request, tracker, today);

  if (request.request_kind === 'equipment_review') {
    return {
      eligible: false,
      availability: 'unavailable',
      reason: 'This request only needs an equipment review.',
    };
  }
  if (!ASSIGNABLE_REQUEST_STATUSES.has(request.status)) {
    return {
      eligible: false,
      availability: 'unavailable',
      reason: 'This request is not open for assignment.',
    };
  }
  if (kit.status === 'maintenance') {
    return {
      eligible: false,
      availability: 'unavailable',
      reason: 'Needs attention before it can be assigned.',
    };
  }
  if (kit.status === 'retired') {
    return {
      eligible: false,
      availability: 'unavailable',
      reason: 'Not in circulation.',
    };
  }
  if (kit.status === 'in_transit') {
    return {
      eligible: false,
      availability: 'unavailable',
      reason: 'Currently in transit.',
    };
  }
  if (
    kit.next_request_id &&
    ...
    return {
      eligible: false,
      availability: 'unavailable',
      reason: 'Reserved for another request.',
    };
  }
  if (kit.status === 'available') {
    if (kit.checked_out_request_id) {
      return {
        eligible: false,
        availability: 'unavailable',
        reason: 'Checkout must be cleared before it can be assigned.',
      };
    }
    return {
      eligible: true,
  ...
  if (kit.status === 'with_holder') {
    if (!kit.checked_out_request_id) {
      return {
        eligible: false,
        availability: 'unavailable',
        reason: 'Current checkout details are incomplete.',
      };
    }
    if (!kit.due_back) {
      return {
        eligible: false,
        availability: 'unavailable',
        reason: 'A return date is required before planning a handoff.',
      };
    }
    if (kit.due_back > shipBy) {
      return {
        eligible: false,
        availability: 'unavailable',
        reason: `Due back ${kit.due_back}, after this request must ship ${shipBy}.`,
      };
    }
    return {
      eligible: true,
      availability: 'direct_handoff',
      reason: `Direct handoff is possible after ${kit.due_back}.`,
    };
  }
  return {
    eligible: false,
    availability: 'unavailable',
    reason: 'Not available for assignment.',
  };
}

/**
 * Returns every physical kit as an assignment choice for one request.
 *
 * Unlike the automation recommendations, these choices describe current
 * inventory only. A kit is never hidden because an earlier recommendation
 * happened to consider it first.
 */
export function getMicKitAssignmentOptions(
  trackerValue,
  requestId,
  options = {}
) {
  ...
  const request = tracker.requests.find(
    (candidate) => candidate.request_id === cleanRequestId
  );
  if (!request) return [];

  const today =
    cleanDate(options.today) || new Date().toISOString().slice(0, 10);
  const dueBack = plannedDueBack(request);
  const choices = tracker.kits.map((kit) => {
    const originCountry = originCountryForKit(tracker, kit);
    const assignment = assignmentAvailability(
      tracker,
      kit,
      request,
      today
    ...
    return {
      ...
      label: kit.label,
      status: kit.status,
      home_country: kit.home_country,
      origin_country: originCountry,
      same_country: Boolean(
        originCountry && request.country && originCountry === request.country
      ),
      eligible: assignment.eligible,
      availability: assignment.availability,
      reason: assignment.reason,
      ship_by: plannedShipBy(kit, request, tracker, today),
      due_back: dueBack,
      current_due_back: kit.due_back,
      shipping_provider:
        originCountry === 'US'
          ? kit.checked_out_request_id
            ? 'pirate_ship_manual'
  ...
  choices.sort(
    (a, b) =>
      ...
      (availabilityOrder[a.availability] ?? 9) -
        (availabilityOrder[b.availability] ?? 9) ||
      Number(b.same_country) - Number(a.same_country) ||
      String(a.current_due_back || '9999').localeCompare(
        String(b.current_due_back || '9999')
      ) ||
  ...
  return choices.map((choice, index) => {
    const isRecommended = choice.eligible && !recommendationSet;
    if (isRecommended) recommendationSet = true;
    return {
      ...choice,
...
function hostsCoveredWithoutARequest(episode = {}) {
...
export function buildMicKitAutomation(
  ...
  const recommendations = tracker.requests
    .filter(
      (request) =>
        ...
        ASSIGNABLE_REQUEST_STATUSES.has(request.status)
    )
    .map((request) =>
  ...
  for (const recommendation of recommendations) {
    ...
    recommendation.assignment_options = getMicKitAssignmentOptions(
      tracker,
    ...
    recommendation.recommended_due_back = plannedDueBack(request);
    recommendation.assignment_options = recommendation.assignment_options.map(
      (option) => ({
        ...option,
        is_recommended: option.kit_id === kit.kit_id,
      })
    );
    if (kit.checked_out_request_id) {
  ...
  for (const request of tracker.requests) {
    ...
    const episode = episodesById.get(request.episode_id);
    const priority = priorityForDate(
      request.recording_date || request.need_by || episode?.recording_date,
      today
    );
    actions.push({
      action_id: `equipment-review-${request.request_id}`,
      urgency: priority.urgency,
      kind: 'review_equipment_plan',
      title: `Confirm ${request.requester_name}’s recording setup`,
      detail: [
        episode?.title ? `Assigned to “${episode.title}”` : '',
        request.notes,
      ]
        .filter(Boolean)
        .join(' · '),
    ...
    if (request?.status === 'assigned') {
      ...
      if (originCountry && originCountry !== 'US') {
        actions.push({
          ...
          detail: `This handoff starts in ${originCountry}, so it cannot use Pirate Ship’s US-origin workflow.`,
          request_id: request.request_id,
          kit_id: kit.kit_id,
        });
        ...
        actions.push({
          ...
          detail:
            'Enter it once on the kit record so future Pirate Ship uploads need less manual editing.',
          request_id: request.request_id,
          kit_id: kit.kit_id,
        });
      }
  ...
  for (const episode of upcomingEpisodes) {
    ...
    const nonRequestCoverage = hostsCoveredWithoutARequest(episode);
    const uncovered = (episode.host_person_ids || []).filter(
      (personId) =>
        !requestPeople.has(personId) && !nonRequestCoverage.has(personId)
    );
