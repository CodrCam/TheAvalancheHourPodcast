...
import { getGuestQuestionnaireUploadMutationLock } from './guestQuestionnaireUploadPolicy.mjs';
...
export function assertGuestQuestionnaireUploadMutationsAllowed(
  questionnaire
) {
  const lock = getGuestQuestionnaireUploadMutationLock(questionnaire);
  if (!lock) return;
  throw new GuestQuestionnaireUploadApiError(lock.message, {
    status: 409,
    code: lock.code,
  });
}
