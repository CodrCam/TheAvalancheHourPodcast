...
function getStoredGuide(item, attributeName, label) {
  ...
  const parsed = parseContentJson(attribute.S, label);
  const storedSchemaVersion = Number(parsed.schema_version);
  const currentSchemaVersion = Number(
    DEFAULT_STUDIO_GUIDE.schema_version
  );
  ...
  if (
    ...
    return {
      guide: normalizedDefaultGuide(),
      source: 'default-upgrade',
      persisted: false,
    };
  }

  return {
    guide: normalizeStudioGuide(parsed, DEFAULT_STUDIO_GUIDE),
    source: 'dynamo',
    persisted: true,
  };
}
...
function resultFromItem(
  ...
  const storedPublished = getStoredGuide(
    item,
    'content_json',
    'published content'
  );
  const publishedGuide = storedPublished?.guide || defaultGuide;
  const publishedSource = storedPublished?.source || 'default';
  ...
  const storedDraft = getStoredGuide(
    item,
    'draft_content_json',
    'draft content'
  );
  const hasDraft = storedDraft?.persisted === true;
  const draftGuide = hasDraft ? storedDraft.guide : publishedGuide;
...
export async function getStudioGuide(options = {}) {
  ...
  const resourcePathIds = Array.isArray(options.resourcePathIds)
