...
import { scrubGuestMicKitDataForEpisode } from './guestQuestionnaireMicKit.mjs';
import { getMicKitTracker, saveMicKitTracker } from './micKitStore.js';

function isConditionalWriteConflict(error) {
  return /conditional|condition.*failed/i.test(
    `${error?.name || ''} ${error?.message || ''}`
  );
}

async function scrubDeletedEpisodeMicKitData(episodeId, options = {}) {
  const loadTracker = options.getMicKitTracker || getMicKitTracker;
  const saveTracker = options.saveMicKitTracker || saveMicKitTracker;
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    const scrubbed = scrubGuestMicKitDataForEpisode(
      result.tracker,
      episodeId
    );
    if (!scrubbed.changed) {
    ...
    try {
      await saveTracker(scrubbed.tracker, {
        expectedUpdatedAt: result.tracker.updated_at,
        updatedBy: 'episode-studio-deletion',
      });
      return {
        configured: true,
        changed: true,
        scrubbed_request_count: scrubbed.scrubbed_request_count,
      };
    } catch (error) {
      if (attempt === 0 && isConditionalWriteConflict(error)) continue;
      throw error;
    }
  }
...
export async function finalizeEpisodeStudioDeletion(
  ...
  await scrubDeletedEpisodeMicKitData(episode.episode_id, options);
