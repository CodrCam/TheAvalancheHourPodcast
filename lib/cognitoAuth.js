...
import { pickStudioDisplayName } from './studioIdentityPresentation.mjs';
...
function getCognitoConfig() {
  ...
  return {
    ...
    studioManagerGroup:
      process.env.COGNITO_STUDIO_MANAGER_GROUP || 'studio_manager',
    hostGroup: process.env.COGNITO_HOST_GROUP || 'host',
...
export function getCognitoIdTokenFromRequest(req) {
  return getCookieValue(req, 'cognito_id_token');
}
...
export async function verifyCognitoAccessToken(token) {
  ...
  if (payload.token_use !== 'access') {
    throw new Error('Cognito access token required');
  }
  return payload;
}

export function getGroupsFromCognitoPayload(payload) {
  const config = getCognitoConfig();
  const tokenGroups = Array.isArray(payload?.['cognito:groups'])
    ? payload['cognito:groups']
    : [];
  const groupMappings = [
    [ACCESS_GROUPS.ADMIN, config.adminGroup],
    [ACCESS_GROUPS.LOGISTICS, config.logisticsGroup],
    [ACCESS_GROUPS.STUDIO_MANAGER, config.studioManagerGroup],
    [ACCESS_GROUPS.HOST, config.hostGroup],
  ];

  return groupMappings
    .filter(([, configuredName]) => tokenGroups.includes(configuredName))
    .map(([canonicalName]) => canonicalName);
}

export function getRoleFromCognitoPayload(payload) {
  return getPrimaryAccessGroup(getGroupsFromCognitoPayload(payload));
}
...
export function getDisplayNameFromCognitoPayload(payload) {
