...
export function normalizeProjectedGuestMicKitPlan(value = {}) {
  ...
  return {
    ...
    choice,
    request_id:
      choice === 'request_kit' ? cleanText(source.request_id, 120) : '',
...
export const DEFAULT_GUEST_QUESTIONNAIRE_QUESTIONS = Object.freeze([
  ...
  question(
    ...
    {
      helpText:
        'This stays visible until both your microphone and headphone setup are confirmed. The producer will confirm availability and timing.',
      required: true,
      sortOrder: 300,
      options: [
...
export function projectGuestQuestionnaireResponse(recordValue = {}) {
  ...
  const equipmentConfirmed =
    microphoneReadiness === 'yes' && headphoneReadiness === 'yes';
  ...
  if (kitDecision === 'yes') {
    ...
    guestEquipmentNote =
      'The guest confirmed a dedicated microphone and suitable headphones.';
    ...
    guestEquipmentNote =
      'The guest asked the episode team to help decide whether a microphone kit is needed.';
  }
  ...
  const guestMicKitPlan = normalizeProjectedGuestMicKitPlan({
    ...
    readiness: {
      internet: answer(record, 'high_speed_internet'),
      microphone: microphoneReadiness,
      headphones: headphoneReadiness,
      quiet_place: answer(record, 'quiet_recording_place'),
    },
  });
...
export function applyGuestQuestionnaireProjectionToEpisode(
  ...
  const packageFields = [
    ...
    ['social-copy', 'social_copy'],
    ['credits', 'credits'],
  ];
  for (const [deliverableId, snapshotField] of packageFields) {
    const deliverableIndex = nextDeliverables.findIndex(
      (deliverable) => deliverable?.id === deliverableId
    );
    if (deliverableIndex < 0) continue;
    const incoming = cleanText(packageProjection[snapshotField], 12000);
    if (!incoming) continue;
    const current = cleanText(nextDeliverables[deliverableIndex]?.value, 12000);
    const previous = cleanText(prior.package[snapshotField], 12000);
    if (!current || (previous && current === previous)) {
  ...
  const nextProductionTasks = productionTasks.map((task) => {
    ...
    if (!current || (previous && current === previous)) {
      nextSnapshotProduction.guest_recording_plan_note =
        incomingRecordingPlanNote;
      appliedFields.push(
        'production_tasks.guest-recording-plan-reviewed.evidence_note'
      );
      return { ...task, evidence_note: incomingRecordingPlanNote };
    }
    skippedFields.push(
      'production_tasks.guest-recording-plan-reviewed.evidence_note'
    );
