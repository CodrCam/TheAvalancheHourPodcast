...
const MAX_STORED_IMAGE_LENGTH = 520000;
...
function normalizeRole(value) {
  ...
  return role === 'producer' ? 'producer' : 'host';
}
  ...
  do {
    const response = await dynamoDbRequest('Scan', {
      ...
      ProjectionExpression:
        '#id, #slug, #role, #name, #title, #images_json, #bio_short, #bio_full, #active, #needs_bio, #needs_images, #sort_order, #updated_at',
...
export async function savePerson(value = {}) {
  ...
  const oversizedImage = (person.images || []).find(
    (image) => image.length > MAX_STORED_IMAGE_LENGTH
  );
  if (oversizedImage) {
    throw new Error('One host image is too large. Please use a smaller image.');
  }
