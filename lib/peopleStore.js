...
export async function getPersonById(personId, options = {}) {
  const cleanPersonId = String(personId || '').trim();
  if (!cleanPersonId) return null;

  const allowStaticFallback = options.allowStaticFallback !== false;
  const includeInactive = options.includeInactive === true;
  const staticFallback = allowStaticFallback
    ? staticPeopleRows().find(
        (person) =>
          person.person_id === cleanPersonId &&
  ...
  const storedPerson = response.Item ? fromDynamoItem(response.Item) : null;
  const visibleStoredPerson =
    storedPerson && (includeInactive || storedPerson.active)
      ? storedPerson
      : null;

  return {
    person: visibleStoredPerson || staticFallback,
    source: visibleStoredPerson ? 'dynamo' : staticFallback ? 'static' : 'dynamo',
    configured: true,
  };
}
