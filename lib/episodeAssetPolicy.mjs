...
export function findDuplicateEpisodeAsset(
  assets = [],
  candidate = {}
) {
  const fileName = sanitizeEpisodeAssetFileName(candidate.file_name)
    .normalize('NFKC')
    .toLocaleLowerCase('en-US');
  const contentType = normalizeContentType(candidate.content_type);
  const size = Math.trunc(Number(candidate.size) || 0);
  const category = String(candidate.category || '').trim();
  const deliverableId = String(candidate.deliverable_id || '').trim();
  if (!fileName || !contentType || size <= 0 || !deliverableId) return null;

  return (
    (Array.isArray(assets) ? assets : []).find(
      (asset) =>
        sanitizeEpisodeAssetFileName(asset.file_name)
          .normalize('NFKC')
          .toLocaleLowerCase('en-US') === fileName &&
        normalizeContentType(asset.content_type) === contentType &&
        Number(asset.size) === size &&
        String(asset.category || '').trim() === category &&
        String(asset.deliverable_id || '').trim() === deliverableId
    ) || null
  );
}
...
export function isProducerProofDeliverable(deliverable = {}) {
  return (
    String(deliverable?.id || '').trim() ===
      PRODUCER_PROOF_DELIVERABLE_ID ||
    deliverable?.allowed_uploader === 'producer'
  );
}

export function canUploadEpisodeAssetToDeliverable({
  deliverable = {},
  roles = [],
  status = '',
  canManage = false,
  ...
  if (isProducerProofDeliverable(deliverable)) {
    const proofTask = (Array.isArray(episode.production_tasks)
      ? episode.production_tasks
      : []
    ).find(
      (task) =>
        String(task?.task_id || task?.id || '').trim() ===
        'producer-proof-upload'
    );
    const isAssignedProofOwner =
      Boolean(viewerPersonId) &&
      (Array.isArray(proofTask?.assigned_person_ids)
        ? proofTask.assigned_person_ids
        : []
      ).includes(viewerPersonId);
    return (
      canManage === true ||
      roles.includes('producer') ||
      isAssignedProofOwner
    );
  }
  return canUploadEpisodeAssets({ roles, status });
}

export function resetProducerProofApprovalForNewAsset(tasks = []) {
  return (Array.isArray(tasks) ? tasks : []).map((task) => {
    const taskId = String(task?.task_id || task?.id || '').trim();
    const isApproval = taskId === 'proof-listen-approval';
    const isDownstream = [
      'publishing-package',
      'promotion-scheduled',
      'guest-assets-shared',
    ].includes(taskId);
    if (!isApproval && !isDownstream) return task;
    ...
    return {
      ...task,
      status:
        isApproval || task.status !== 'not_started'
          ? 'in_progress'
          : 'not_started',
      ...(isApproval ? { proof_decision: 'pending' } : {}),
...
export function getProducerProofUploadDependencyBlockers(episode = {}) {
  ...
  const proofTask = taskById.get('producer-proof-upload');
  if (!proofTask) return [];
  return (Array.isArray(proofTask.dependencies)
    ? proofTask.dependencies
    : []
  ).filter((dependencyId) => {
    const dependency = taskById.get(String(dependencyId || '').trim());
    return (
      !dependency ||
      !isEpisodeProductionTaskComplete(dependency, episode)
    );
  });
}

export function canDeleteEpisodeAsset({
  roles = [],
  status = '',
  canManage = false,
  viewerPersonId = '',
  ...
  if (
    roles.includes('workflow_assignee') &&
    Boolean(viewerPersonId) &&
    viewerPersonId === uploaderPersonId
  ) {
    return true;
  }
  if (!canUploadEpisodeAssets({ roles, status })) return false;
  if (roles.includes('producer')) return true;
  return (
    roles.includes('host') &&
    Boolean(viewerPersonId) &&
    viewerPersonId === uploaderPersonId
...
export function canReadEpisodeAsset({
  roles = [],
  canManage = false,
  viewerPersonId = '',
  episode = {},
  ...
  if (canManage) return true;
  const assignedRoles = new Set(Array.isArray(roles) ? roles : []);
  if (assignedRoles.has('host') || assignedRoles.has('producer')) {
    return true;
  }
  if (!assignedRoles.has('workflow_assignee') || !viewerPersonId) {
    return false;
  }
  const assetId = String(asset.asset_id || '').trim();
  const deliverableId = String(asset.deliverable_id || '').trim();
  return (Array.isArray(episode.production_tasks)
    ? episode.production_tasks
    : []
  ).some((task) => {
    const assigned = Array.isArray(task?.assigned_person_ids)
      ? task.assigned_person_ids
      : [];
    if (!assigned.includes(viewerPersonId)) return false;
    const linkedDeliverables = Array.isArray(task?.linked_deliverable_ids)
      ? task.linked_deliverable_ids
      : [];
    return (
      (Boolean(deliverableId) && linkedDeliverables.includes(deliverableId)) ||
      (Boolean(assetId) && task?.evidence_asset_id === assetId)
    );
  });
}

export function filterEpisodeAssetsForViewer(
  episodeValue = {},
  access = {}
) {
  return {
    ...episodeValue,
    assets: (Array.isArray(episodeValue.assets)
      ? episodeValue.assets
      : []
    ).filter((asset) =>
      canReadEpisodeAsset({ ...access, episode: episodeValue, asset })
    ),
  };
}
