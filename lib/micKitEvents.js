...
function entry(request, type, title, preview, token, options = {}) {
  ...
  if (request.requester_person_id === options.actorPersonId) return null;
  return {
  ...
  return [...new Set((managerPersonIds || []).filter(Boolean))]
    ...
    .map((personId) => ({
      ...
      notification: {
        ...
        actor_person_id: actorPersonId,
        entity_kind: 'mic_kit',
        entity_id: kit.kit_id,
        group_entity_kind: 'mic_kit',
        group_entity_id: kit.kit_id,
        group_key: `mic-kit:${kit.kit_id}`,
        deep_link: `/admin/mic-kits#${kit.kit_id}`,
        required_permission: 'mic_kits:manage',
        audit: {
          source_action: base.sourceAction || '',
          recipient_reason: 'mic_kit_manager',
        },
...
function micKitOperationsEntries(
  managerPersonIds,
  actorPersonId,
  base,
  dedupePrefix
  ...
  return [...new Set((managerPersonIds || []).filter(Boolean))]
    ...
    .map((personId) => ({
      ...
      notification: {
        ...
        actor_name: base.actorName || '',
        actor_person_id: actorPersonId,
        entity_kind: 'mic_kit_inventory',
        entity_id: 'shared',
        group_entity_kind: 'mic_kit_inventory',
        group_entity_id: 'shared',
        group_key: 'mic-kit-inventory:shared',
        deep_link: '/admin/mic-kits',
        required_permission: 'mic_kits:manage',
        audit: {
          source_action: base.sourceAction || '',
          recipient_reason: 'mic_kit_manager',
...
export function buildMicKitNotificationEntries({
    ...
    for (const request of tracker.requests) {
      ...
      if (
        ...
        (previous.status !== request.status ||
          ...
          previous.request_kind !== request.request_kind ||
          previous.review_resolution !== request.review_resolution)
      ) {
        const result = entry(
          request,
        ...
        entries.push(
          ...managerEntries(
            ...
            actorPersonId,
            {
              type: 'mic_kit_request_updated',
              title: `${request.requester_name || 'A host'}’s mic kit request was updated`,
              preview:
                request.admin_response ||
                `The request is now ${request.status.replace(/_/g, ' ')}.`,
              actorName,
              sourceAction: action,
            },
            `mic-kit:manager-request-updated:${request.request_id}:${request.admin_updated_at || token}`
          )
        );
      }
    }
      ...
      entries.push(
        ...managerEntries(
          ...
          actorPersonId,
          {
            ...
            preview:
              'The shared operations queue and any planned handoff were updated.',
            actorName,
            sourceAction: action,
          },
  ...
  if (action === 'confirm_inventory') {
    entries.push(
      ...micKitOperationsEntries(
        managerPersonIds,
        actorPersonId,
        {
          type: 'mic_kit_inventory_confirmed',
          title: 'The shared mic kit inventory was confirmed',
          preview:
            'The circulation count and inventory note were confirmed on the operations board.',
          actorName,
          sourceAction: action,
        },
        `mic-kit:inventory-confirmed:${token}`
      )
    );
  }
  ...
  if (
    [
      ...
      'assign_request_to_kit',
      'checkin_kit',
    ].includes(action)
  ) {
    const changedKit = (tracker.kits || []).find((kit) => {
      const previous = previousKits.get(kit.kit_id);
      return !previous || JSON.stringify(previous) !== JSON.stringify(kit);
    });
    if (changedKit) {
      const actionCopy = {
        add_kit: {
          type: 'mic_kit_added',
          title: `${changedKit.label} was added`,
          preview:
            'A mic kit was added to the shared circulation inventory.',
        },
        update_kit: {
          type: 'mic_kit_updated',
          title: `${changedKit.label} was updated`,
          preview:
            'Location, status, assignment, or shipment details changed on the operations board.',
        },
        checkout_kit: {
          type: 'mic_kit_checked_out',
          title: `${changedKit.label} was checked out`,
          preview:
            'Custody and the active request were updated on the operations board.',
        },
        apply_recommendation: {
          type: 'mic_kit_assignment_applied',
          title: `${changedKit.label} was assigned`,
          preview:
            'The recommended handoff was applied to the operations board.',
        },
        assign_request_to_kit: {
          type: 'mic_kit_assignment_applied',
          title: `${changedKit.label} was assigned`,
          preview:
            'A coordinator selected this kit from the eligible inventory choices.',
        },
        checkin_kit: {
          type: 'mic_kit_checked_in',
          title: `${changedKit.label} was checked in`,
          preview:
            'The kit return and next availability were recorded.',
        },
      }[action];
      entries.push(
        ...micKitManagerEntries(
          managerPersonIds,
          changedKit,
          actorPersonId,
          {
          ...
          `mic-kit:${action}:${changedKit.kit_id}:${token}`
        )
      );
    }
  }
