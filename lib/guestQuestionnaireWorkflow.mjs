...
export function reopenGuestQuestionnaireWorkflowForUpdate(
  episodeValue = {},
  {
    actorPersonId = '',
    actorName = 'Guest questionnaire',
  ...
  for (const [taskId, note] of [
    [
      GUEST_QUESTIONNAIRE_RECEIVED_TASK_ID,
      'A corrected guest response was requested.',
    ],
    [
      GUEST_QUESTIONNAIRE_SENT_TASK_ID,
      'A new private guest update link was created and is waiting to be shared.',
    ],
  ]) {
    const task = (Array.isArray(episode.production_tasks)
      ? episode.production_tasks
    ...
    if (!task || !['complete', 'waived'].includes(task.status)) continue;
    episode = applyEpisodeProductionTaskUpdate(
      episode,
      taskId,
