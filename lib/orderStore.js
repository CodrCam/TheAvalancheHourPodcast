...
async function deleteOrderFromDynamo(orderId) {
  const tableName = getOrdersTableName();
  ...
  return response.Attributes ? fromDynamoItem(response.Attributes) : null;
}
...
async function markInventoryDecrementFailedInDynamo(orderId) {
  const tableName = getOrdersTableName();
  const now = new Date().toISOString();
  await dynamoDbRequest('UpdateItem', {
    TableName: tableName,
    Key: { order_id: { S: String(orderId) } },
    UpdateExpression:
      'SET #inventory_decrement_status = :failed, #updated_at = :updated_at',
    ConditionExpression:
      'attribute_exists(#order_id) AND #inventory_decrement_status = :processing',
    ExpressionAttributeNames: {
      '#order_id': 'order_id',
      '#inventory_decrement_status': 'inventory_decrement_status',
      '#updated_at': 'updated_at',
...
function assertDynamoOrdersReady() {
  if (!shouldUseDynamo()) {
...
export async function deleteOrder(orderId) {
  ...
  assertDynamoOrdersReady();
  return deleteOrderFromDynamo(orderId);
}
...
export async function markInventoryDecrementFailed(orderId) {
  assertDynamoOrdersReady();
  return markInventoryDecrementFailedInDynamo(orderId);
}
