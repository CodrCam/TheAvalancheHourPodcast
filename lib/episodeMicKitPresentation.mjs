...
export function buildEpisodeMicKitPlanRows({
  ...
  if (!hasEpisodeGuestMicKitPlan(guestPlan)) return hostRows;

  const normalizedGuestPlan = normalizeEpisodeGuestMicKitPlan(guestPlan);
  const linkedGuestCoverage = (
    ...
    (coverage) =>
      ...
      cleanId(coverage?.participant_type) === 'guest'
  );
  ...
  return [
    ...hostRows,
    {
      ...
      ...normalizedGuestPlan,
      resolved:
        normalizedGuestPlan.choice === 'request_kit'
          ? Boolean(
              safeGuestCoverage &&
                isActiveEpisodeMicKitRequestCoverage(safeGuestCoverage)
            )
          : isEpisodeGuestMicKitPlanResolved(normalizedGuestPlan),
