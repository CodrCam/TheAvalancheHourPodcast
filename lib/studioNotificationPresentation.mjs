...
export function notificationCanBeOpened(
  ...
  if (notification.category === 'mic_kit') {
    if (
      permissionSet.has('mic_kits:manage') &&
      notification.required_permission === 'mic_kits:manage'
    ) {
      if (notification.group_entity_kind === 'mic_kit_inventory') {
        return true;
      }
      if (notification.group_entity_kind === 'mic_kit') {
        const kit = micKitsById.get(notification.group_entity_id);
        return Boolean(kit && !kit.deleted_at);
      }
    }
