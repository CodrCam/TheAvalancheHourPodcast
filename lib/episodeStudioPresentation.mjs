...
const EPISODE_ASSET_CATEGORIES = [
  'recording',
  'image',
  'document',
  'sponsor_audio',
...
export const DEFAULT_EPISODE_DELIVERABLES = [
  ...
  },
  {
    ...
    description:
      'Upload research, releases, reference documents, or other source material the producer may need. Drive can remain a working space, but the final source files belong here.',
    type: 'asset',
    required: false,
    asset_category: 'document',
    sort_order: 30,
...
function normalizeDeliverable(value = {}, index = 0) {
  const type = ['url', 'asset'].includes(value.type)
    ? value.type
    : 'textarea';
  const assetCategory = EPISODE_ASSET_CATEGORIES.includes(
