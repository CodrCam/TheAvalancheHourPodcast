...
export const EPISODE_STUDIO_SCHEMA_VERSION = 10;
...
export const DEFAULT_EPISODE_DELIVERABLES = [
  ...
  },
  {
    ...
    description:
      'Upload every final local conversation track after Riverside or the recording platform finishes processing it. If you record the introduction yourself, upload that finished intro here too. Keep filenames clear and distinct; no intro file is required when you send the script and schedule the producer recording.',
    type: 'asset',
    required: true,
  ...
  },
  {
    ...
    description:
      'Provide one clear source brief for the final show notes and promotion: the episode summary, title ideas, two to four key takeaways, guest biography, verified public links and handles, suggested excerpts or timestamps, and any no-tag, privacy, or do-not-publish instructions. Put the actual images, artwork, captions, and visual guidance in Photos and artwork, and record formal creator credits and permissions in Credits and permissions. The publishing owner drafts the final public copy.',
    type: 'textarea',
    required: true,
    asset_category: 'document',
    sort_order: 60,
  },
  {
    ...
    description:
      'Previous intro-only uploads are retained here for existing episodes. New recorded introductions belong with Raw recording tracks.',
    type: 'asset',
    required: false,
  ...
  },
  {
    ...
    description:
      'Upload the promotion image source material here. Collect 5–6 clearly named candidate images when possible, then choose and order exactly three final images. Include caption and visual guidance, flag crop or editing needs, and confirm the creator credit, permission, and anything to avoid.',
    type: 'asset',
    required: true,
  ...
  'show-notes': Object.freeze({
    ...
    descriptions: Object.freeze({
      'Give Sierra and Angie the episode summary, takeaways, guest links and handles, image guidance, credits, and anything that must not be published.':
        DEFAULT_DELIVERABLES_BY_ID.get('show-notes').description,
  ...
  photos: Object.freeze({
    ...
    descriptions: Object.freeze({
      'Upload 2–6 clearly named images. Identify the cover, preferred order, intended use, crop, caption, credit, permission, and anything to avoid in the producer handoff.':
        DEFAULT_DELIVERABLES_BY_ID.get('photos').description,
...
function positiveInteger(value, fallback = 0) {
  ...
  return Number.isInteger(fallbackCandidate) && fallbackCandidate > 0
    ? fallbackCandidate
    : 0;
}

function appendDistinctText(primaryValue, secondaryValue, heading = '') {
  const primary = cleanText(primaryValue, 12000);
  const secondary = cleanText(secondaryValue, 12000);
  if (!secondary || primary === secondary || primary.includes(secondary)) {
    return primary;
  }
...
function consolidatePromotionSourceMaterial(deliverables = []) {
  const source = Array.isArray(deliverables) ? deliverables : [];
  const promotion = source.find(
    (deliverable) => cleanId(deliverable?.id) === 'social-copy'
  );
  if (!promotion) return source;

  const showNotesIndex = source.findIndex(
    (deliverable) => cleanId(deliverable?.id) === 'show-notes'
  );
  const showNotesTemplate = DEFAULT_DELIVERABLES_BY_ID.get('show-notes');
  const existingShowNotes =
    showNotesIndex >= 0 ? source[showNotesIndex] : showNotesTemplate;
  const mergedShowNotes = {
    ...showNotesTemplate,
    ...existingShowNotes,
    id: 'show-notes',
    required:
      existingShowNotes?.required !== false || promotion.required !== false,
    value: appendDistinctText(
      existingShowNotes?.value,
      promotion.value,
      'Additional promotion source material'
    ),
    social_profiles: appendDistinctText(
      existingShowNotes?.social_profiles,
      promotion.social_profiles
    ),
    legacy_source_url:
      existingShowNotes?.legacy_source_url || promotion.legacy_source_url || '',
    missing_acknowledged:
      existingShowNotes?.missing_acknowledged === true ||
      promotion.missing_acknowledged === true,
    missing_note: appendDistinctText(
      existingShowNotes?.missing_note,
      promotion.missing_note,
      'Promotion source-material gap'
    ),
    expected_by:
      existingShowNotes?.expected_by || promotion.expected_by || '',
    sort_order: existingShowNotes?.sort_order || showNotesTemplate.sort_order,
  };

  if (
    existingShowNotes?.legacy_source_url &&
    promotion.legacy_source_url &&
    existingShowNotes.legacy_source_url !== promotion.legacy_source_url
  ) {
    mergedShowNotes.value = appendDistinctText(
      mergedShowNotes.value,
      promotion.legacy_source_url,
      'Previous promotion source link'
    );
  }

  const withoutPromotion = source.filter(
    (deliverable) => cleanId(deliverable?.id) !== 'social-copy'
  );
  if (showNotesIndex >= 0) {
    return withoutPromotion.map((deliverable) =>
      cleanId(deliverable?.id) === 'show-notes'
        ? mergedShowNotes
        : deliverable
    );
  }
  return [...withoutPromotion, mergedShowNotes];
}
...
export function normalizeEpisodeStudio(value = {}, fallback = {}) {
  ...
  const migratedSourceDeliverables = consolidatePromotionSourceMaterial((
  ...
  const normalizedAssets = (
    ...
    .map((asset) => {
      if (asset.deliverable_id === 'social-copy') {
        return {
          ...asset,
          deliverable_id:
            asset.category === 'image' ? 'photos' : 'show-notes',
        };
      }
  ...
  return {
    ...
    source_mastermind_plan_id: Object.prototype.hasOwnProperty.call(
      value,
      'source_mastermind_plan_id'
    )
      ? cleanId(value.source_mastermind_plan_id)
      : cleanId(fallback.source_mastermind_plan_id),
    source_mastermind_plan_revision: Object.prototype.hasOwnProperty.call(
      value,
      'source_mastermind_plan_revision'
    )
      ? positiveInteger(value.source_mastermind_plan_revision)
      : positiveInteger(fallback.source_mastermind_plan_revision),
...
export function getEpisodeStudioMembership(value = {}, identity = {}) {
  ...
  if (personId && episode.production_lead_person_id === personId) {
    membership.push('production_lead');
  }
  if (
...
export function removeEpisodeAssetFromEpisode(
  ...
  const introTask = episode.production_tasks.find(
    (task) => task.task_id === 'intro-ready'
  );
  const removedCurrentIntro =
    introTask?.intro_method === 'recorded' &&
    introTask.evidence_asset_id === cleanAssetId;
  const resetIntroTasks = removedCurrentIntro
    ? episode.production_tasks.map((task) =>
        task.task_id === 'intro-ready'
          ? {
              ...task,
              status: task.status === 'waived' ? 'waived' : 'in_progress',
              completed_at: '',
  ...
  const resetProofDependents = removedCurrentProducerProof
    ? resetProducerProofApprovalForNewAsset(resetIntroTasks)
    : resetIntroTasks;
  return normalizeEpisodeStudio({
    ...episode,
    deliverables,
