...
export const EPISODE_STUDIO_SCHEMA_VERSION = 3;
...
export const DEFAULT_EPISODE_DELIVERABLES = [
  ...
  },
  {
    ...
    description:
      'Write the listener-facing episode summary, topics covered, and public links. Do not repeat private guest contact details here.',
    type: 'textarea',
    required: true,
    asset_category: 'document',
    sort_order: 60,
...
export function getEpisodeCompletion(value = {}) {
  ...
  const completed = required.filter((deliverable) =>
    isDeliverableComplete(deliverable, completionAssets)
  );
...
export function removeEpisodeAssetFromEpisode(value = {}, assetId = '') {
  const episode = normalizeEpisodeStudio(value);
  const cleanAssetId = cleanId(assetId);
  if (!cleanAssetId) return episode;
  return normalizeEpisodeStudio({
    ...episode,
    assets: episode.assets.filter((asset) => asset.asset_id !== cleanAssetId),
