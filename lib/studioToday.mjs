...
function episodeAction(episode, options) {
  ...
  const dueDate = episode.due_date || episode.target_release_date || '';
  ...
  return items
    ...
    .map((item) => {
      ...
      return {
        ...
        badge: blocker ? 'Blocker' : isNew ? 'Team Inbox · new' : 'Team Inbox',
...
export function buildStudioToday(
  ...
  const episodeActions = (Array.isArray(episodes) ? episodes : [])
    .map((episode) =>
      episodeAction(episode, { today, canManageEpisodes })
    )
    .filter(Boolean);
  ...
  const activeEpisodes = episodes.filter(
    (episode) =>
      !episode.archived && ACTIVE_EPISODE_STATUSES.has(episode.status)
  );
  ...
  return {
    ...
    metrics: {
      ...
      off_track: activeEpisodes.filter(
        (episode) => episode.delivery_health === 'off_track'
      ).length,
