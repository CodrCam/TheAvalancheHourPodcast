...
export function isViewerMicKitRequestActionable(request = {}) {
  const belongsToViewer = request.is_mine === true;
  const coordinatedGuest =
    request.participant_type === 'guest' && request.is_coordinator === true;
  return Boolean(
    (belongsToViewer || coordinatedGuest) &&
      ACTIVE_MIC_REQUEST_STATUSES.has(request.status)
  );
}
...
export function filterStudioTodayActions(
  actions = [],
  filter = 'priority',
  options = {}
) {
  ...
  if (filter === 'due_this_week') {
    return rows.filter((action) => {
      const days = daysBetween(today, action?.date);
      return days !== null && days >= 0 && days <= 7;
    });
  }
  if (filter === 'operations') {
    return rows.filter((action) => action?.kind === 'operations');
  }
  return rows.slice(0, PRIORITY_QUEUE_LIMIT);
}
...
function episodeAction(episode, options) {
  ...
  if (
    ...
    (options.canManageEpisodes || myRoles.has('host'))
  ) {
    score += isOffTrack ? 80 : 65;
    urgency = isOffTrack ? 'urgent' : urgency === 'urgent' ? urgency : 'high';
...
function micKitActions(payload = {}, canManageMicKits = false, today = '') {
  if (canManageMicKits) {
  ...
  return (payload.tracker?.requests || [])
    .filter(isViewerMicKitRequestActionable)
    .map((request) => {
      const equipmentReview =
        request.participant_type === 'guest' &&
        request.request_kind === 'equipment_review' &&
        request.status === 'requested';
      const reviewPriority = equipmentReview
        ? priorityForDate(request.need_by || request.recording_date, today)
        : null;
      const copy = {
      ...
      if (equipmentReview) {
        const guestName = String(request.requester_name || '').trim();
        return {
          id: `mic-kit:${request.request_id}`,
          kind: 'mic_kit',
          title: guestName
            ? `Confirm ${guestName}’s recording setup`
            : 'Confirm the guest’s recording setup',
          detail:
            request.notes ||
            'The guest’s questionnaire needs an equipment decision before recording.',
          badge: 'Guest equipment review',
          date: request.need_by || request.recording_date || '',
          urgency:
            reviewPriority?.urgency === 'low'
              ? 'medium'
              : reviewPriority?.urgency || 'medium',
          score: Math.max(55, reviewPriority?.score || 0),
          href: `/studio/mic-kits#${encodeURIComponent(request.request_id)}`,
        };
      }
...
export function buildStudioToday(
  ...
  return {
    date: today,
    actions: filterStudioTodayActions(actions, 'priority', { today }),
    all_actions: actions,
    ...
    metrics: {
      ...
      due_this_week: filterStudioTodayActions(actions, 'due_this_week', {
        today,
      }).length,
