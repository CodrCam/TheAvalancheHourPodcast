...
import { isGuestQuestionnaireTokenConfigured } from './guestQuestionnaireToken.mjs';
...
export async function getAdminSystemHealth() {
  ...
  const guestQuestionnaireLinksOk = isGuestQuestionnaireTokenConfigured();
  const guestQuestionnaireUploadsOk = isEpisodeAssetStorageConfigured();
  ...
  const checks = [
    ...
    check(
      'guest_questionnaire_links',
      'Guest questionnaire links',
      guestQuestionnaireLinksOk,
      guestQuestionnaireLinksOk ? 'Configured' : 'Needs setup',
      guestQuestionnaireLinksOk
        ? 'Private, expiring guest links can be created.'
        : 'GUEST_QUESTIONNAIRE_TOKEN_SECRET is missing or too short.',
      guestQuestionnaireLinksOk ? 'good' : 'warn'
    ),
    check(
      'guest_questionnaire_uploads',
      'Guest questionnaire uploads',
      guestQuestionnaireUploadsOk,
      guestQuestionnaireUploadsOk ? 'Configured' : 'Needs setup',
      guestQuestionnaireUploadsOk
        ? 'Guest resumes and photos can use protected episode storage.'
        : 'Episode asset storage must be configured before sharing a questionnaire with upload fields.',
      guestQuestionnaireUploadsOk ? 'good' : 'warn'
    ),
    check(
      'last_order',
      'Last order recorded',
