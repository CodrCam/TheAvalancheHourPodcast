...
export function getStudioNotificationSetupIssue(error) {
  const message = String(error?.message || error || '');
  if (
    /not authorized to perform:\s*dynamodb:query/i.test(message) &&
    /\/index\/|index/i.test(message)
  ) {
    return {
  ...
  if (
    /specified index.*(?:not found|does not exist)|table does not have the specified index|resource not found/i.test(
      message
    )
  ) {
    return {
