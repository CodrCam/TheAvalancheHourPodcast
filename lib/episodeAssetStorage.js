  ...
  const policy = Buffer.from(
    JSON.stringify({
      ...
      conditions: [
        ...
        { key: objectKey },
        { 'Content-Type': contentType },
        // S3 applies this condition to the multipart/form-data request, which
        // also contains the signed fields and browser-generated boundaries.
        // The completion step still HEAD-checks the stored object's exact
        // authorized size and MIME type before attaching it to the episode.
        [
          'content-length-range',
          size,
          size + S3_POST_FORM_OVERHEAD_ALLOWANCE_BYTES,
        ],
...
async function signedS3Request(method, objectKey, additionalQuery = []) {
  const config = storageConfig();
  if (!isEpisodeAssetStorageConfigured()) {
    throw new Error('Episode asset storage is not configured.');
  }
...
export async function deleteEpisodeAssetObject(
  ...
  const response = await signedS3Request('DELETE', cleanObjectKey, [
