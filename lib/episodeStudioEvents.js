...
export function buildEpisodeNotificationEntries({
  ...
  if (previousEpisode && action === 'update') {
    ...
    if (
      ...
      previousEpisode.recording_date !== episode.recording_date ||
      previousEpisode.recording_time !== episode.recording_time ||
      previousEpisode.recording_time_zone !== episode.recording_time_zone ||
      previousEpisode.recording_duration_minutes !==
        episode.recording_duration_minutes ||
      previousEpisode.recording_location !== episode.recording_location
    ) {
      entries.push(
        ...recipientEntries(
          participants(episode),
          actorPersonId,
          {
            ...
            preview: episode.recording_date
              ? `Recording is scheduled for ${episode.recording_date}. Open the Episode Studio for the exact time and calendar download.`
              : episode.due_date
  ...
  if (action === 'asset_uploaded' && event.asset?.asset_id) {
    const isProducerProof =
      event.asset.deliverable_id === 'producer-proof-audio';
    entries.push(
      ...recipientEntries(
        participants(episode),
        actorPersonId,
        {
          ...
          preview: isProducerProof
            ? 'A new private producer proof is ready to download and review in the Production workspace.'
            : event.asset.deliverable_id
          ...
          deep_link: isProducerProof
            ? `${productionDeepLink}#producer-proof`
            : `${deepLink}#final-assets`,
        },
    ...
    entries.push(
      ...recipientEntries(
        ...
        actorPersonId,
        {
          ...
          preview: isProducerProof
            ? 'The private producer proof was removed. Upload a current proof before host approval can continue.'
            : event.asset.deliverable_id
      ...
      entries.push(
        ...recipientEntries(
          ...
          actorPersonId,
          {
            ...
            deep_link: `${productionDeepLink}#production-handoff`,
            urgency: 'high',
            intent: 'actionable',
          },
    ...
    entries.push(
      ...recipientEntries(
        ...
        actorPersonId,
        {
          ...
          preview:
            'The production team flagged the current delivery outlook. Open the production workflow for context and recovery steps.',
          deep_link: `${productionDeepLink}#production-workflow`,
          urgency: 'high',
          intent: 'urgent',
        },
