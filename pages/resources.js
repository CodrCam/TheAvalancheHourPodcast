...
export default function Resources() {
  ...
  const generalResources = [
    {
      ...
      description: 'Research papers and proceedings from international snow science conferences'
    },
    {
      name: 'American Avalanche Association',
      link: 'https://www.americanavalancheassociation.org/',
      description: 'Professional organization for avalanche workers in the United States'
    },
    {
  ...
  const nationalCenters = [
    {
      ...
      description: 'Coordinating body for avalanche information in the United States'
    },
    {
  ...
  const filterStates = () => {
    ...
    return Object.keys(avalancheCentersByState).filter(state => {
      ...
      return hasMatchingCenter || state.toLowerCase().includes(searchQuery.toLowerCase());
    });
  };
      ...
      <Container maxWidth="lg" sx={{ mt: 4, mb: 6 }}>
        ...
        <Box sx={{ mb: 6 }}>
          ...
          <Grid container spacing={3}>
            {filterCenters(nationalCenters).map((center) => renderCenterCard(center))}
          </Grid>
        </Box>
            ...
            <List>
              {generalResources
                ...
                >
                  <ListItemText 
                    primary={
                      <Typography className="resource-name" sx={{ fontWeight: 600 }}>
                        {resource.name}
                      </Typography>
                    }
        ...
        <Box sx={{ mb: 4 }}>
          ...
          <Typography variant="body1" color="text.secondary" sx={{ mb: 4 }}>
            Find avalanche forecasts and conditions for your region. Centers are organized by state 
            and provide critical safety information for backcountry travelers.
          </Typography>
            ...
            >
              ...
              >
                <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
                  <Typography variant="h5" component="h3" sx={{ flexGrow: 1, fontWeight: 600 }}>
                    {state}
                  </Typography>
                  <Chip 
                    label={`${filterCenters(avalancheCentersByState[state]).length} center${filterCenters(avalancheCentersByState[state]).length !== 1 ? 's' : ''}`}
                    color="primary"
                    variant="outlined"
                    size="small"
                  />
              ...
              <AccordionDetails sx={{ p: 3 }}>
                <Grid container spacing={3}>
                  {filterCenters(avalancheCentersByState[state]).map((center) => renderCenterCard(center))}
                </Grid>
              </AccordionDetails>
            </Accordion>
          ))}
          ...
          <Box sx={{ display: 'flex', alignItems: 'flex-start' }}>
            ...
            <Box>
              ...
              <Typography variant="body2" color="text.secondary">
                Always check current avalanche conditions before heading into the backcountry. 
                Carry proper rescue equipment (beacon, probe, shovel), travel with experienced partners, 
                and consider taking an avalanche safety course. No forecast is a substitute for good judgment and proper preparation.
              </Typography>
            </Box>
          </Box>
        </Paper>
