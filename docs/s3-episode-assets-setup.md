...
accepted. Completion reads S3's authoritative upload time, and Episode Studio
records and displays the corresponding date, stops offering downloads after
that date, and generates a participant reminder when the date is within 30
days. Keep noncurrent episode versions for the same 180-day window: downloads
are pinned to the exact version completion verified, and a replayed upload must
not shorten that promised retention period.
